...
author: ~
date: '2019-09-27'
slug: predicting-t-shirt-size-from-height-and-weight
categories: ['Prediction']
tags: ['R', 'regression', 'shirts', 'body measurements']
disableComments: no
---

Today I was given a task that sounded so straight-forward: What t-shirt size would you send to someone if you don't know their shirt size, but instead you know their height, weight, and gender? 

It was so straight-forward that I was **sure** there **must** be prior art out there that I could re-use. A StackOverflow, a mathematical formula, a GitHub repo, a blog post -- there had to be something! To my surprise, there wasn't any, not that I could find anyway. 

It seems like this is a problem that hasn't received a lot of attention.

So I did what a data scientist does: I set out to build my own predictive algorithm. 
...
Now my model was complete. My model would be a linear regression that predicts chest circumference from gender, height and weight (or bmi). I would train this model on the NHANES data I would then the predicted chest circumference to determine shirt size.
...
I decided to build two models. Why? Sometimes I know a person's height and weight but not their BMI, and other times I know their BMI but not their height and weight. Although I could take all of the heights and weights and convert them into BMIs, I'm guessing this leads to some loss of information. Plus, the correlation above suggests that weight is the best predictor of chest size (better than BMI), so that tells me it's better to use height and weight when it's available, but BMI as a fallback.
...
![](Gildan_Unisex_Sizing_Chart.png)
