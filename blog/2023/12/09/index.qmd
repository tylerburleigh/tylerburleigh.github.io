...
date: 2023-12-09
description: "In this post, I show how an AI Math Tutor feature like \"step-by-step guidance\" can be generated using GPT and prompt engineering. Validating the step-by-step guidance, I find that GPT's accuracy increased from 39% to 77% when solving the problem using its own step-by-step instructions."
...
Imagine you're developing an AI Math Tutor application: You have a set of math questions that student users can answer, along with the correct answers to those questions. Students will sometimes get stuck answering multi-step problems, and in those situations you want them to ask the AI tutor for help. One AI Tutoring interaction that can be helpful in such a situation is to provide students with step-by-step guidance.

![Simple AI Math Tutor mock-up (mouse cursor is from kindpng.com)](mockup.png)

In this blog post, I show how a "step-by-step guidance" feature could be developed using GPT, some prompt engineering, and a multi-step math problem that GPT-3.5 struggles to answer. I also show how this step-by-step guidance can be validated by asking GPT to solve the problem using its own instructions and then comparing its "stepwise" performance against a baseline model.
...
tibble(`Baseline Solver` = 39, `Stepwise Solver` = 77)%>% 
  ...
  ggplot(aes(x = name, y = value, fill = name)) + 
    ...
    labs(
      ...
      title = 'Validating the step-by-step guidance, performance of the Stepwise Solver was\nroughly double performance of the Baseline Solver.'
    ) +
    ylim(0, 100) +
