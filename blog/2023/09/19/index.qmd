---
title: 'Five methods of categorical encoding: one-hot, frequency, label, target, and embedding'
date: 2023-09-08
...
holdout <- read_csv('data/playground-series-s3e22/test.csv', show_col_types = F)
sample_submission <- read_csv('data/playground-series-s3e22/sample_submission.csv', show_col_types = F)
```

# Explore the dataset

The training dataset contains character and numeric columns. There are no missing values.

```{r, message=F}
df %>% lares::df_str(return = "plot")
```

Looking at the character columns, I do see possible missing values. For example, it seems like a "None" for `temp_of_extremities` could be a missing value. Also I see some opportunities for data cleaning: Some columns have both "None" and "none". Converting all strings to lowercase would help to at least combine the "none" types.

```{r}
char_cols <- colnames(df %>% select(is.character))
for(col in char_cols){
  print(paste0(toupper(col), ': ', paste0(distinct(df[col])[[1]], collapse=', ')))
}
```

There is some degree of class imbalance.

```{r}
df %>%
  group_by(outcome) %>%
  count() %>%
  mutate(pct = round(n/nrow(df)*100))
```
A few features have strong correlations.
...
df %>% mutate_if(is.character, .funs=tolower) %>%
...
The first -- and most popular -- type of categorical encoding is one-hot encoding. One-hot encoding transforms a single categorical variable with N levels into binary variables encoding each of the N levels.
...
freq_encoding <- encodeR::frequency_encoder(
  X_train = train,
  X_test = test, 
  cat_columns = colnames(df %>% select(is.factor, -outcome))
)

train_freq <- my_encodings$train
test_freq <- my_encodings$test
```
...
For target encoding (also called "effect encoding", "likelihood encoding", or "bayesian encoding"), I'll be using the `h2o` package because it supports multi-class targets. The `embed` package can also do target encoding and integrates better with a tidymodels workflow, but at the moment it only supports binary targets.
...
With target encoding, the levels of the categorical variable are replaced with the mean on the target, when grouped by that level. Because the outcome is being used for encoding, care needs to be taken when using this method to avoid leakage and overfitting. In this case, I'll use the "Leave One Out" method: for each row, the mean is calculated over all rows excluding that row. (And, of course, I'll train the encoder on the training set and not the test set.)

```{r, message=F}
# Choose which columns to encode
encode_columns <- colnames(df %>% select(is.factor, -outcome)) # All categorical variables
...
```{r}
char_cols <- colnames(train %>% select(is.factor, -outcome))
cols_for_onehot <- c()
cols_for_embdding <- c()
for(col in char_cols){
...
'll define three models to evaluate: multinomial logistic regression, random forest, and xgboost.
...
I'll run each of the models using the `fit_encodings()` and `fit_model()` functions that I defined earlier.
...
From this summary, I can see that among the categorical encodings: target performed best on average, frequency performed worst on average, but the best models used the onehot or onehot + embedding encoding strategies. And in terms of ML algorithm, the two tree-based models performed better, on average, as compared to the logistic model.
